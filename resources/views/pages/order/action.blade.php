...
<div class="btn-group">
		...
		@elseif ($row->order_status_id == 2)
			@if(Auth::user()->type->id == 3)
				<a class="btn btn-primary"  href="{{ route('order.package', ['id' => $row->order_id]) }}">Бэлтгэх</a>
			@else
		...
		@elseif($row->order_status_id == 3)
			...
			@else
				<a class="btn btn-primary"  href="{{ route('order.shipping_started', ['id' => $row->order_id]) }}">Хүргэлтэн гарсан</a>
			@endif
